...
class TestComplementoPaternidadService:
    ...
    def test_check_eligibility_period_1_incapacidad_invalid(self):
        ...
        assert result.period == PeriodType.PERIOD_1
        assert "Período 1 solo aplica para pensiones de jubilación" in result.reason
    ...
    def test_check_eligibility_period_2_viudedad_invalid(self):
        """Test elegibilidad período 2 con viudedad (no válida)."""
        result = self.service.check_eligibility(
        ...
        assert result.eligible == False
        assert result.period == PeriodType.PERIOD_2
        assert "Período 2 solo aplica para jubilación e incapacidad" in result.reason
    
    def test_calculate_complement_period_1_two_children(self):
        """Test cálculo período 1 con 2 hijos (2%)."""
        ...
        assert result.period == PeriodType.PERIOD_1
        assert result.complement_percent == 2.0
        assert result.complement_fixed is None
        assert result.amount == 20.0  # 2% de 1000€
        assert result.pension_with_complement == 1020.0
    
    def test_calculate_complement_period_1_three_children(self):
        """Test cálculo período 1 con 3 hijos (3%)."""
        result = self.service.calculate_complement(
            PensionType.JUBILACION,
            date(2020, 6, 15),
            3,
        ...
        assert result.period == PeriodType.PERIOD_1
        assert result.complement_percent == 3.0
        assert result.amount == 45.0  # 3% de 1500€
        assert result.pension_with_complement == 1545.0
    ...
    def test_calculate_complement_period_2_one_child(self):
        ...
        assert result.complement_fixed == 35.0
        assert result.amount == 35.0  # 35€ por hijo
        assert result.pension_with_complement == 1035.0
    ...
    def test_calculate_complement_period_2_four_children(self):
        ...
        assert result.period == PeriodType.PERIOD_2
        assert result.complement_fixed == 35.0
        assert result.amount == 140.0  # 4 × 35€
        assert result.pension_with_complement == 1640.0
    ...
    def test_calculate_complement_period_2_more_than_four_children(self):
        ...
        assert result.period == PeriodType.PERIOD_2
        assert result.amount == 140.0  # máximo 4 × 35€
        assert result.pension_with_complement == 1640.0
